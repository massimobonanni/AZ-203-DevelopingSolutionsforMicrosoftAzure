...
Your company builds an HR system that is used by various customers around the world. While the system works fine today, your development managers have decided to begin rearchitecting the solution by decoupling application components. This decision was driven by a desire to make any future development simpler due to modularity. As the developer who handles component communication, you have decided to introduce Azure Event Grids as your solution-wide messaging platform.
...
-   Publish a message from a .NET Core application
...
1.  In the **Settings** section, go to the **Properties** section, and then record the value of the **URL** box. You'll use this value later in the lab.
...
1.  Return to **Azure Event Grid Viewer** web application.
...
1.  In the **Settings** section, go to the **Access keys** section, and then record the value of the **Key 1** box. You'll use this value later in the lab.
...
1.  Using a terminal, create a new .NET Core project named **EventPublisher** in the current folder:
...
1.  In the **Main** method, perform the following actions:
    ...
    1.  Add the following block of code to create a new variable named **firstPerson** of type **Person**, create a new variable named **firstEvent** of type **EventGridEvent**, populate the **EventGridEvent** variable with sample data and then add the **firstEvent** instance to your **events** list:
    ...
    1.  Add the following block of code to create a new variable named **secondPerson** of type **Person**, create a new variable named **secondEvent** of type **EventGridEvent**, populate the **EventGridEvent** variable with sample data and then add the **secondEvent** instance to your **events** list:
