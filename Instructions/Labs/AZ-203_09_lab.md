...
Your organization keeps a collection of JSON files used to configure third-party products in an SMB file share in Azure. As part of a regular auditing practice, the ops team would like to call a simple HTTP endpoint and retrieve the current list of configuration files. You have decided to implement this functionality using a no-code solution based on Azure API Management and Logic Apps.
...
1.  Access the **prodstor\*** storage account that you created earlier in this lab.
...
1.  In the **metadata** share, select **Upload** to upload all five of the JSON files in the **Allfiles (F): \\Allfiles\\Labs\\09\\Starter** folder on your lab VM.
...
1.  Access the **prodflow\*** logic app that you created earlier in this lab.
...
1.  Access the **prodapim\*** API Management resource that you created earlier in this lab.
...
1.  Create a new **Logic App** integrated API with the following details:
    ...
    -   Description: **Lookup metadata JSON files**
...
1.  Start the **httprepl** tool and set the base URI to the value of the **Request URL** for the API operation that you copied earlier in this lab.
